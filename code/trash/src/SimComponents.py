...
class Packet(object):
    ...
    def __init__(self, time, size, id, src="a", dst="z", flow_id=0):
        ...
        #explicit path created 
        #now find a way to set multiple outs on switch ports (dict)
        #send packets from out based on explicit paths
        #generate a method in Network_Component to identify the out interface from present router and next router
...
class PacketGenerator(object):
    ...
    def __init__(self, env, id,  adist, sdist, initial_delay=0, finish=float("inf"), flow_id=0):
        ...
        self.finish = finish
        ''''''
        self.out = {} #next network component vs interface
        self.packets_sent = 0
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            p = Packet(self.env.now, self.sdist(), self.packets_sent, src=self, dst=random_dest, flow_id=self.flow_id)

            next_hop = p.next_hop()
            if(next_hop in self.out): #else drop packet
                yield self.env.timeout(self.adist())
                self.packets_sent += 1
                p.increment_explicit_path_pointer()
                self.out[next_hop].put(p)
    ...
    def run(self):
        while True:
            ...
            if(next_hop in self.out): #else drop packet 
                ...
                self.out[next_hop].put(msg)
                self.busy = 0
                if self.debug:
                    print msg

    def put(self, pkt):
        #if pkt.verify_current_position():
        self.packets_rec += 1
