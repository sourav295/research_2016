...
class PacketGenerator(object):
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            #JUST TO ENSURE MTU is satisfied
            MTU = GlobalConfiguration.MTU
            pkt_size = MTU + 1
            while pkt_size > MTU:
                pkt_size = self.sdist()
        
        
            p = Packet(self.env.now, pkt_size, self.packets_sent, src=self, dst=random_dest, flow_id=self.flow_id, priority=random_prio)
            simulation_logger.info("Generated By: {:4} ID: {:4} Msg: [{}]".format(self, p.id, p))
            self.packets_sent += 1
            #transmission delay - this is configurable
            if GlobalConfiguration.simulate_delay_on_server:
...
class SwitchPort(object):
    ...
    def addInterface(self, next_hop):
        self.routing_processor.add_interface(next_hop, self, self.rate)
