...
class PacketGenerator(object):
    ...
    def run(self):
        ...
        while self.env.now < self.finish:
            ...
            print "Generated By: ", self," ID: ", p.id, " time: ", self.env.now 
            self.packets_sent += 1
    ...
    def run(self):
        ...
        while True:
            ...
            if not self.selector or self.selector(msg):
                ...
                if self.debug:
                    print "Received @",self.env.now,"By: ",self,msg
                    print msg.explicit_path
...
class SwitchPort(object):
    ...
    def run(self):
        while True:
            '''msg = (yield self.store.get())'''
            store = (self.routing_processor.get(self))
            msg = (yield store.get())
            ''''''
            self.busy = 1
    ...
    def put(self, pkt):
        ...
        if self.qlimit is None:
            self.byte_size = tmp
            return self.routing_processor.put(pkt)
        ...
        if tmp >= self.qlimit:
            ...
            self.byte_size = tmp
            return self.routing_process.put(pkt)
            '''return self.store.put(pkt)'''
